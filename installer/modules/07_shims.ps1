...
$shimLines = @(
    ...
    "set `"ARG=%~1`"",
    "if /i `"!ARG!`"==`"-p`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"--prompt`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG:~0,9!`"==`"--prompt=`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"--continue`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"-r`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"--resume`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG:~0,9!`"==`"--resume=`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"--session-id`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG:~0,13!`"==`"--session-id=`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"-h`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"--help`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"help`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"init`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"login`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"logout`" set `"SKIP_BOOTSTRAP=1`"",
    "if /i `"!ARG!`"==`"update`" set `"SKIP_BOOTSTRAP=1`"",
    "set `"ARGS=!ARGS! `"%~1`"`"",
    "shift & goto parse",
    ":run",
    ...
    "cd /d `"$WorkDir`"",
    "if `"!ARGS!`"==`"`" ($EngramCopilotCmd -i `"!ENGRAM_BOOTSTRAP!`") else if `"!SKIP_BOOTSTRAP!`"==`"1`" ($EngramCopilotCmd !ARGS!) else ($EngramCopilotCmd -i `"!ENGRAM_BOOTSTRAP!`" !ARGS!)"
)
...
$ClaudeCommandSource = Join-Path $ProjectRoot ".github\skills\engram-claude\engram.md"
if (Test-Path $ClaudeCommandSource) {
    ...
    Write-Warn "Claude command source not found: $ClaudeCommandSource"
}
