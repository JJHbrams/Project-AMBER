...
캐릭터를 **내장 볼따구**로 선택하면 `매핑 편집…`에서 상태·도구 범주·런처 전환에 연결할 포즈를 고를 수 있다. 선택 상자를 바꾸면 편집기 아래의 미리보기가 배포본에 포함된 아틀라스와 같은 레이어·프레임 시간으로 반복 재생된다. 미리보기는 외부 렌더러나 별도 로컬 서버를 시작하지 않는다.

`가져오기…`는 JSON의 지원되는 항목만 검증한 뒤 Engram 소유 복사본으로 가져오며 원본 파일을 바꾸지 않는다. 편집기의 `적용 준비`는 변경된 선택만 담은 소유 매핑 파일을 준비할 뿐이다. 이어서 **설정 창의 저장**을 눌러야 선택된 매핑 경로가 사용자 설정에 기록되고, 오버레이를 다시 시작한 뒤 반영된다. 숨겨진 유효 항목(예: 1회 재생 정책)은 다른 포즈를 편집해도 보존된다.
