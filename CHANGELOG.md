...
- 말풍선 모드를 작업표시줄에 노출되지 않는 네이티브 WebView 창으로 교체했다.
  입력 풍선은 `Shift+Enter` 줄바꿈, 클립보드 이미지 첨부, 전송 대기 큐와
  수정·취소·인터럽트 후 전송을 지원한다.
- 입력창과 큐를 같은 크기의 풍선 안에서 뒤집어 전환하고, 최근 전송 항목은
  입력창 위의 작은 슬롯에서 취소하거나 인터럽트할 수 있다.
- Claude와 Codex의 상주 말풍선 세션을 각각 연결하고 provider별 resume ID와
  제목 메타데이터를 분리해 보존한다.

### Changed

- 응답·생각 풍선은 내용 길이에 따라 설정된 최대 크기 안에서 동적으로 변하며,
  설정 GUI의 글꼴과 크기를 사용한다. 사용자가 조절한 상대 위치는 오버레이 이동
  후에도 유지되고, 응답 풍선은 운영체제 크기 조절을 지원한다.
- 입력 풍선의 불필요한 설명과 여백을 줄이고, 본체와 꼬리를 하나의 실루엣으로
  렌더링한다. 표시 설정만 바꿀 때는 진행 중인 provider 세션을 끊지 않는다.

### Validation

- `1.5.16.740` frozen 번들과 설치기는 Windows clean build, runtime contract,
  역할별 smoke, 실제 네이티브 WebView 크기·드래그·OS resize 검증을 통과했다.
- 실제 provider별 IME·이미지·승인·인터럽트 흐름과 다중 DPI 시각 검증은 환경별
  후속 검증 대상으로 남긴다.
...
- 볼따구를 외장 프로세스가 아닌 Engram 네이티브 기본 캐릭터로 제공한다.
  기존 외장 `mapping.json` 호환과 네이티브 매핑 편집을 지원하며, 편집기는
  배포 atlas의 실제 레이어·프레임 타이밍으로 선택 동작을 미리 보여준다.
- 설치기에 2D/3D 외장 오버레이 7종의 계층형 복수 선택과 독립 SDK 선택을
  추가했다. 설치와 활성 캐릭터 선택은 분리하고, 필요한 공유 의존성만 설치한다.
- 외부 세션 모니터링의 선택·제목 복구·도구/자식 활동 연결과 startup 연동을
  개선했다.

### Installation

- 기존 설치 항목과 사용자 소유 설정·매핑을 보존한다. 체크 해제는 삭제가 아니다.
- 고정 버전과 SHA-256으로 외장 구성요소를 검증하며, 실패한 세대 전환은 이전
  runtime을 보존한다.
...
- 1.5.13 의 소유권 판정이 provenance 기록만 보아서, 그 이전 배치가 깔아둔
  `planner`/`coder`/`servant` 가 전부 "사용자 것"으로 분류되어 건너뛰어졌다.
  정의가 그 시점 내용으로 얼어붙어 이후 개선이 소스 설치 사용자에게 도달하지
  않는 문제였다. 디스크 내용이 배포 원본과 바이트 동일하면 사용자 작업물이
  아니므로 기록을 남기고 진행한다. 내용이 다르면 1.5.13 과 동일하게 건드리지 않는다.

### 핫픽스 적용 방법

- **소스에서 `INSTALL.ps1` 로 설치한 경우** — 이 경로만 영향을 받았다.
  `git pull` 후 `INSTALL.ps1` 을 다시 실행하면 된다. 배치 스크립트는 checkout
  에서 직접 실행되므로 별도 조치가 없다.
- **AMBER installer 로 설치한 경우** — 할 일이 없다. 이 배치는
  `modules/07_shims.ps1` 에서만 호출되므로 설치본은 이 경로를 타지 않았다.
- **이미 덮어써진 파일은 되돌려지지 않는다.** 1.5.13 이전에는 백업을 남기지
  않았다. 소스 설치 이력이 있고 자신의 `planner`/`coder`/`servant` 를 가지고
  있었다면 내용을 확인할 것.
- 관리 정의를 의도적으로 최신으로 되돌리려면 `-Force` 를 준다. 그때는
  `.engram-bak` 으로 백업한 뒤 덮어쓴다.
...
- 설정 GUI에서 선택한 캐릭터 소스 방식이 이전 단일 이미지·애니메이션 폴더 경로보다
  우선 적용되어, `스프라이트 그리드`로 바꾸면 즉시 올바른 sprite 캐릭터가 표시된다.
- 단일 이미지의 본체 크기와 위치는 기본적으로 고정하면서 VFX는 그대로 표시한다.
  이전 늘림·상하 이동 효과는 `단일 이미지 레거시 움직임` 옵션으로 다시 켤 수 있다.
- 번들 캐릭터 리소스를 `static`, `sequences`, `sets`, `reactions` 구조로 정리하고 중복
  이미지를 제거했다. 기존 번들 상대경로와 현재 설치 위치의 구형 경로도 안전하게 호환한다.
- 1.5 이전 설정에서 사용자가 선택한 단일 이미지·애니메이션 폴더를 업데이트 후에도
  유지한다. 캐릭터를 바꾸지 않은 신규·기본 사용자는 새 스프라이트 그리드를 사용한다.
