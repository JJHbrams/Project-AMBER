...
class DirectiveRegistrationTests(unittest.TestCase):
    ...
    def test_no_directive_before_explicit_approval_and_one_time_commit(self):
        ...
        draft = self.r.begin_registration()
        completed = self.r.complete_registration(draft["draft_id"], self.payload())
        conn = get_connection()
        self.assertIsNone(conn.execute("SELECT 1 FROM directives WHERE key = ?", (self.key,)).fetchone())
        conn.close()
        preview = self.r.preview_registration(draft["draft_id"])
        approval = self.r.approve_registration(draft["draft_id"], preview["digest"], True)
        committed = self.r.commit_registration(draft["draft_id"], completed["digest"], approval["approval_token"])
        self.assertEqual(committed["status"], "directive_committed")
        with self.assertRaises(self.r.RegistrationError):
            self.r.commit_registration(draft["draft_id"], preview["digest"], approval["approval_token"])

    def test_invalid_and_conditional_values_are_rejected(self):
        draft = self.r.begin_registration()
        with self.assertRaises(self.r.RegistrationError):
            self.r.complete_registration(draft["draft_id"], self.payload(scope="codex"))
        with self.assertRaises(self.r.RegistrationError):
            self.r.complete_registration(draft["draft_id"], self.payload(enforcement_level="blocking", workflow_skill_id="", guard_id="bogus"))
        with self.assertRaises(self.r.RegistrationError):
            self.r.complete_registration(draft["draft_id"], self.payload(trigger_type="conditional", trigger_data={}))

    def test_preview_digest_binds_approval_and_detects_target_change(self):
        draft = self.r.begin_registration()
        self.r.complete_registration(draft["draft_id"], self.payload())
        preview = self.r.preview_registration(draft["draft_id"])
        with self.assertRaises(self.r.RegistrationError):
            self.r.approve_registration(draft["draft_id"], "wrong", True)
        approval = self.r.approve_registration(draft["draft_id"], preview["digest"], True)
