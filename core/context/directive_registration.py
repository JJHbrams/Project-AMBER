...
from core.context.guard_execution import _GUARD_REGISTRY
...
SCHEMA = {
    ...
    "workflow_skill_id": {"required_when": {"enforcement_level": "workflow"}},
...
def registration_schema() -> dict[str, Any]:
    ...
    schema["guard_id"]["choices"] = [
        ...
        for guard_id in sorted(_GUARD_REGISTRY)
    ]
...
def canonicalize(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if raw["enforcement_level"] == "workflow" and not raw["workflow_skill_id"]:
        raise RegistrationError("workflow_skill_id is required for workflow enforcement")
    ...
    if raw["enforcement_level"] == "blocking":
        if raw["guard_id"] not in _GUARD_REGISTRY:
...
def begin_registration(initial: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    try:
        with conn:
            conn.execute("INSERT INTO directive_registration_drafts (draft_id, payload, status, created_at, updated_at) VALUES (?, ?, 'draft', ?, ?)", (draft_id, _canonical_json(initial), now, now))
    finally:
        conn.close()
    return {"status": "draft_started", "draft_id": draft_id, "schema": registration_schema(), "payload": initial}


def complete_registration(draft_id: str, fields: dict[str, Any]) -> dict[str, Any]:
    conn = get_connection()
    try:
...
def preview_registration(draft_id: str) -> dict[str, Any]:
    conn = get_connection()
    try:
        row = conn.execute("SELECT * FROM directive_registration_drafts WHERE draft_id = ?", (draft_id,)).fetchone()
        if row is None or not row["digest"] or row["status"] != "draft":
...
def approve_registration(draft_id: str, digest: str, approved: bool) -> dict[str, Any]:
    if approved is not True:
        raise RegistrationError("explicit approved=true is required")
    conn = get_connection()
    try:
        row = conn.execute("SELECT * FROM directive_registration_drafts WHERE draft_id = ?", (draft_id,)).fetchone()
        if row is None or row["status"] != "draft" or not row["digest"] or not secrets.compare_digest(str(row["digest"]), str(digest)):
...
def commit_registration(draft_id: str, digest: str, approval_token: str) -> dict[str, Any]:
    conn = get_connection()
    try:
        row = conn.execute("SELECT * FROM directive_registration_drafts WHERE draft_id = ?", (draft_id,)).fetchone()
