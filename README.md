...
_호박이 수백만 년의 생명을 보존하듯, AMBER는 AI의 기억과 정체성을 보존합니다._
...
2. `~/.engram/overlay.user.yaml` 의 `discord:` 섹션 설정
...
discord:
  ...
  # 특정 채널/길드는 provider를 고정
  channel_cli_overrides:
    "CHANNEL_ID_A": "gemini"
  guild_cli_overrides:
    "GUILD_ID_2": "ollama"

  # 차단 목록 (allowlist보다 우선)
  deny_channel_ids: []
  deny_user_ids: []

  # 채널별 FIFO + 채널 간 병렬 처리 정책
  queue:
    max_per_channel: 8
    ttl_seconds: 180
...
- DM: 멘션 없이도 응답합니다.
- 서버 채널: 봇 멘션이 있어야 응답합니다.
- 세션: 채널 단위로 지속되며 `/new`, `/newsession`, `/새세션`으로 명시적 새 세션 전환이 가능합니다.
- 세션 명령: `/session`, `/session list [N]`, `/session use <id>`, `/session new [질문]`
- provider 선택 우선순위:
  1. `channel_cli_overrides[channel_id]`
  2. `guild_cli_overrides[guild_id]`
  3. 오버레이 기본 provider (`cli.provider`/runtime state)

### 운영 팁

- 설정 변경 후 오버레이(또는 Discord 봇)를 재시작해야 반영됩니다.
- 대기열이 길어지면 봇이 순번 안내를 보내고, TTL 초과 요청은 만료 안내를 반환합니다.
- 로그 파일: `~/.engram/overlay.log`
