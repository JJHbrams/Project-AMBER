...
def _get_port():
    from core.config.runtime_config import get_cfg_value   # ← 지연 import
    ...

def do_POST(self):
    from core.memory.bus import memory_bus                   # ← 지연 import
    from core.graph.semantic import maybe_promote            # ← 지연 import
```
...
| 모듈                   | 역할           | 사용 위치             |
| ---------------------- | -------------- | --------------------- |
| `core.context.context_builder` | 컨텍스트 조립  | backend.py            |
| `core.storage.db`              | DB 연결        | 전반                  |
| `core.identity`        | 정체성         | mcp_server            |
| `core.memory`          | 기억 저장      | stm_server.do_POST    |
| `core.context.directives`      | 지침           | mcp_server            |
| `core.identity.reflection`     | 반성           | mcp_server            |
| `core.identity.curiosity`      | 궁금증         | mcp_server            |
| `core.common.sanitizer`        | 입력 정제      | mcp_server            |
| `core.memory.bus`              | 세션 파사드    | stm_server.do_POST    |
| `core.config.runtime_config`   | 설정 포트 읽기 | stm_server.\_get_port |
| `core.graph.semantic.stm_promoter` | STM→LTM 승격   | stm_server.do_POST    |
| `core.observability.activity`  | 활동 로그      | mcp_server            |
| `core.context.project_scope`   | scope 해석     | mcp_server            |
