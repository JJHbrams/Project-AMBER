...
_CONTEXT_ONCE_KEYS: "OrderedDict[str, None]" = OrderedDict()
...
@engramMCP.tool()
def engram_get_context(user_query: str = "", caller: str = "all", scope_key: str = "", project_key: str = "", cwd: str = "") -> str:
    ...
    ctx = memory_bus.compose_prompt_context(
        user_query,
        caller=caller,
...
def engram_get_context_once(
    ...
    with _CONTEXT_ONCE_LOCK:
        if cache_key in _CONTEXT_ONCE_KEYS:
            return "[engram] context already initialized for this request session key."
        _CONTEXT_ONCE_KEYS[cache_key] = None
        if len(_CONTEXT_ONCE_KEYS) > _CONTEXT_ONCE_MAX:
            _CONTEXT_ONCE_KEYS.popitem(last=False)

    return engram_get_context(
...
def _schedule_memories_sync() -> None:
    ...
    def _run():
        try:
            ...
            # sync_all_ep_to_kg는 여기서 제거 (upsert_episode 내부에서 이미 link_episode_to_kg 호출)
        except Exception:
            pass
...
_sync_watchdog_thread = threading.Thread(
    target=_sync_watchdog_loop, daemon=True, name="engram-sync-watchdog"
)
...
def _schedule_post_session_sync() -> None:
    ...
    if now - _last_sync_completed_at < cooldown:
        _logger.debug(
            ...
            now - _last_sync_completed_at, cooldown,
        )
        return
...
@engramMCP.tool()
def engram_save_message(session_id: int, role: str, content: str, request_id: str = "") -> dict:
    ...
    # fallback: 직접 SQLite
    if role == "user":
        memory_bus.record_user_message(session_id, content)
    elif role == "assistant":
        memory_bus.record_assistant_message(session_id, content)
    else:
...
def kg_read_note(identifier: str) -> dict:
    ...
    filepath = _Path(vault_path) / rel_path
