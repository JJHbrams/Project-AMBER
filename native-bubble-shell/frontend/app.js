...
function flip(on){if(composing){flipPending=on;return;}queueOpen=on;if(!on&&$('draft'))requestAnimationFrame(()=>{$('draft').focus();$('draft').setSelectionRange(...rememberedSelection);});else if($('draft'))rememberedSelection=[$('draft').selectionStart,$('draft').selectionEnd];$('rotor').classList.toggle('flipped',on);$('inputFace').inert=on;$('queueFace').inert=!on;$('inputFace').setAttribute('aria-hidden',String(on));$('queueFace').setAttribute('aria-hidden',String(!on));resize();if(on)setTimeout(()=>$('queue').focus(),250);}
function previews(){const nodes=attachments.map((url,i)=>{const wrap=document.createElement('div');wrap.className='preview';const img=document.createElement('img');img.src=url;img.alt=`첨부 이미지 ${i+1}`;const b=button('remove','첨부 이미지 제거');b.disabled=!!pending;b.onclick=()=>{attachments.splice(i,1);previews();resize();};wrap.append(img,b);return wrap;});$('previews').replaceChildren(...nodes);}
...
function schedulePresentationMeasure(){
 if(label==='input'||measurePending)return;
 measurePending=true;
 requestAnimationFrame(()=>{
  ...
  const face=document.querySelector('#bubble>.face');if(!face)return;
...
function renderPresentation(){const p=state[label]||{};window.AmberText.render($('content'),p.text||'');document.body.classList.toggle('nudge',label==='speech'&&!!p.nudge);const key=JSON.stringify([p.presentation_revision,p.fade_ms,state.approval?.id]);if(key!==presentationKey){presentationKey=key;setPresentationFade(p);}const approval=state.approval;if(approval)clearPresentationFade();let old=$('approval');if(old)old.remove();if(label==='speech'&&p.nudge&&!approval){const choices=document.createElement('div');choices.id='approval';choices.className='choices';for(const [actionName,text] of [['nudge_reply','답장'],['nudge_defer','나중에']]){const b=document.createElement('button');b.textContent=text;b.onclick=()=>{b.disabled=true;action(actionName,{presentation_revision:p.presentation_revision});};choices.append(b);}$('content').after(choices);}else if(label==='speech'&&approval){const box=document.createElement('div');box.id='approval';box.className='approval';const text=document.createElement('p');text.textContent=approval.summary;const choices=document.createElement('div');choices.className='choices';for(const [v,t] of [[false,'거부'],[true,'허용']]){const b=document.createElement('button');b.textContent=t;b.onclick=()=>{b.disabled=true;action('approval',{approval_id:approval.id,allow:v});};choices.append(b);}box.append(text,choices);$('content').after(box);}}
...
async function init(){
 ...
 const resizeHandle=document.createElement('button');resizeHandle.className='resize-handle';resizeHandle.setAttribute('aria-label','말풍선 크기 조절');resizeHandle.textContent='⌟';resizeHandle.onpointerdown=e=>{e.preventDefault();invoke('shell_resize').catch(()=>{});};document.body.append(resizeHandle);
 if(label!=='input'){$('bubble').innerHTML=`<section class="face"><header class="drag"><strong>${label==='speech'?'AMBER':'생각 중…'}</strong><button id="dismiss" aria-label="말풍선 닫기" data-tip="말풍선 닫기">×</button></header><div id="content" class="content"></div></section>`;$('utility').hidden=true;$('dismiss').onclick=()=>action('dismiss',{window:label,presentation_revision:presentationFade.revision});}
 else{$('flip').onclick=()=>flip(true);$('back').onclick=()=>flip(false);$('send').onclick=submit;$('resume').onclick=()=>action('resume_queue');$('history').onclick=()=>action('history');$('close').onclick=()=>action('close');$('cancelEdit').onclick=()=>action('cancel_edit',{},editing);$('draft').oncompositionstart=()=>composing=true;$('draft').oncompositionend=()=>{composing=false;if(flipPending!==null){const on=flipPending;flipPending=null;flip(on);}};$('draft').onkeydown=e=>{if(e.key==='Enter'&&!composing&&!e.isComposing&&e.keyCode!==229&&!e.shiftKey){e.preventDefault();submit();}};$('draft').oninput=()=>{resize();clearTimeout(activityTimer);action('input_activity',{active:true});activityTimer=setTimeout(()=>action('input_activity',{active:false}),1000);};$('draft').addEventListener('paste',async e=>{const files=[...e.clipboardData.files].filter(f=>/^image\/(png|jpeg)$/.test(f.type));if(!files.length)return;e.preventDefault();if(pending)return;for(const file of files){if(attachments.length+readers>=4||file.size>5*1024*1024){notice('이미지는 최대 4개, 개별 5 MiB까지 첨부할 수 있습니다.');break;}readers++;try{const url=await new Promise((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(r.result);r.onerror=reject;r.readAsDataURL(file);});attachments.push(url);}catch{notice('이미지를 읽지 못했습니다.');}finally{readers--;}previews();resize();}});$('queue').onwheel=e=>{e.preventDefault();const items=state.queue||[];const n=items.findIndex(i=>i.request_id===selected);selected=items[Math.max(0,Math.min(items.length-1,n+(e.deltaY>0?1:-1)))]?.request_id;render();};}
 ...
 document.body.onmouseenter=()=>{pausePresentationFade();pauseRecentFade();action('hover',{window:label,active:true});};document.body.onmouseleave=()=>{resumePresentationFade();resumeRecentFade();action('hover',{window:label,active:false});};
 await listen('host-message',e=>{const m=e.payload;if(m.type==='snapshot'){state=m.state||{};render();}else if(m.type==='presentation'){state={...state,...m.event};render();}});
 await listen('host-geometry',e=>geometry(e.payload.rect||{}));
 await invoke('shell_ready');render();
}
